...
import { ListMusic, ExternalLink, ChevronDown } from 'lucide-react';
import { YouTubePlaylist, COUNTRIES } from '../types';
  ...
  return (
    <div className="bg-card border border-card-border rounded-2xl overflow-hidden animate-pulse">
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="h-3 bg-secondary/30 rounded-full w-2/3" />
      </div>
    </div>
  );
}

function PlaylistCard({ playlist, index }: { playlist: YouTubePlaylist; index: number }) {
  return (
    <motion.a
    ...
    >
      <div className="relative aspect-video overflow-hidden">
        ...
        <div className="absolute bottom-2 left-2 bg-black/70 rounded-md px-2 py-0.5 text-xs text-white font-medium flex items-center gap-1">
      ...
      <div className="p-4">
        ...
        <p className="text-muted-foreground text-xs mt-1.5 truncate">{playlist.channelTitle}</p>
        {playlist.description && (
          <p className="text-muted-foreground text-xs mt-1 line-clamp-1 opacity-70">
            {playlist.description}
          </p>
        )}
      </div>
...
export default function Playlists() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/youtube/playlists?${params}`)
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 pb-32 min-h-[100dvh]">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Browse curated music playlists from around the world.</p>
      </div>
      ...
      {error ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {isLoading
            ? Array.from({ length: 12 }).map((_, i) => <SkeletonPlaylistCard key={i} />)
            : playlists.length > 0
            ? playlists.map((playlist, i) => <PlaylistCard key={playlist.playlistId} playlist={playlist} index={i} />)
            : (
              <div className="col-span-full py-20 text-center bg-card rounded-2xl border border-border">
                ...
                <p className="text-muted-foreground">Try selecting a different country or search term.</p>
              </div>
            )
          }
        </div>
